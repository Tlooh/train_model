...
def main():
    ...
    def preprocess_train(examples):
        ...
        examples["rm_input_ids"] = reward_model.blip.tokenizer(examples[caption_column], padding='max_length', truncation=True, max_length=35, return_tensors="pt").input_ids
        examples["rm_attention_mask"] = reward_model.blip.tokenizer(examples[caption_column], padding='max_length', truncation=True, max_length=35, return_tensors="pt").attention_mask
        return examples
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                latents = torch.randn((args.train_batch_size, 4, 64, 64), device = accelerator.device)
                ...
                latent_model_input = latents
                latent_model_input = noise_scheduler.scale_model_input(latent_model_input, timesteps[mid_timestep])
                noise_pred = unet(
                    latent_model_input,
                    timesteps[mid_timestep],
                    encoder_hidden_states=encoder_hidden_states,
                ).sample
                pred_original_sample = noise_scheduler.step(noise_pred, timesteps[mid_timestep], latents).pred_original_sample.to(weight_dtype)
                ...
                # image: [bsz, 3, 224, 224]
                # rm_input_ids [bsz, 35]
                # rm_attention_mask [bsz, 35]

                rewards = reward_model.score_gard(batch["rm_input_ids"], batch["rm_attention_mask"], image)
